...
;;;; 3-dimensional "ridged" multifractal fractal noise generator

(defpackage #:%mfiano.graphics.procgen.cricket.generators.ridged-multifractal-3d
  (:local-nicknames
...
(in-package #:%mfiano.graphics.procgen.cricket.generators.ridged-multifractal-3d)

(defstruct (gen:ridged-multifractal-3d
            (:include int:sampler)
            (:conc-name "")
            (:predicate nil)
            (:copier nil))
...
(defun gen:ridged-multifractal-3d (&key seed (generator #'gen:open-simplex2s-3d) (octaves 4)
...
(defmethod int:sample ((sampler gen:ridged-multifractal-3d) x &optional (y 0d0) (z 0d0) (w 0d0))
  (declare (ignore w))
