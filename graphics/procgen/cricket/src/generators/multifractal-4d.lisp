...
;;;; 4-dimensional multifractal fractal noise generator

(defpackage #:%mfiano.graphics.procgen.cricket.generators.multifractal-4d
  (:local-nicknames
...
(in-package #:%mfiano.graphics.procgen.cricket.generators.multifractal-4d)

(defstruct (gen:multifractal-4d
            (:include int:sampler)
            (:conc-name "")
            (:predicate nil)
            (:copier nil))
...
(defun gen:multifractal-4d (&key seed (generator #'gen:open-simplex2s-4d) (octaves 4)
...
octaves (optional, default 0.5)."
  (unless (typep octaves '(integer 1 32))
    (error 'int:invalid-fractal-octave-count :sampler-type 'multifractal-4d :value octaves))
  (unless (realp frequency)
    (error 'int:invalid-real-argument
           :sampler-type 'multifractal-4d
           :argument :frequency
           :value frequency))
...
(defmethod int:sample ((sampler gen:multifractal-4d) x &optional (y 0d0) (z 0d0) (w 0d0))
