...
(u:fn-> make-tile-enabler-counts (core) simple-array)
  ...
  (let* ((adjacencies (adjacencies core))
    ...
    (dotimes (pattern-id pattern-count)
      (loop :with direction-counts = (make-array 4 :element-type 'u:ub8 :initial-element 0)
            ...
            :do (setf (aref direction-counts i) count)
            :finally (setf (aref enabler-counts pattern-id) direction-counts)))
    enabler-counts))
  ...
  (let* ((enabler-counts (enabler-counts tile))
    ...
    (flet ((backtrack/restore-enabler-count (count)
      ...
      (record core
              ...
              (aref direction-counts direction-index))
      (setf (aref direction-counts direction-index) value))
    nil))
