...
(u:fn-> take-snapshot (core function &optional t) null)
(declaim (inline take-snapshot))
(defun take-snapshot (core func &optional arg)
  (declare (optimize speed))
  (when (eq (strategy core) :backtrack)
    (vector-push-extend (cons func arg) (snapshots (backtracker core))))
  nil)

(u:fn-> take-snapshot/ban-pattern (core tile u:ub32) null)
(defun take-snapshot/ban-pattern (core tile pattern-id)
  (declare (optimize speed))
  (flet ((%unban-pattern (data)
           (setf (sbit (possible-patterns tile) pattern-id) data))
         (%restore-weights (data)
           (destructuring-bind (weight . weight-log-weight) data
             (setf (weight tile) weight
                   (weight-log-weight tile) weight-log-weight))))
    (take-snapshot core #'%unban-pattern (sbit (possible-patterns tile) pattern-id))
    (take-snapshot core #'%restore-weights (cons (weight tile) (weight-log-weight tile)))
    nil))
...
(defun take-snapshot/collapse-tile (core tile possible-patterns)
  ...
  (flet ((%uncollapse (data)
    ...
    (when possible-patterns
      (take-snapshot core #'%restore-patterns))
    nil))
