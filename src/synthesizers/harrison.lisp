...
(u:fn-> update-neighbors (state (simple-array u:ub32) u:ub8 u:ub32) null)
...
(u:fn-> update-candidates (state (simple-array u:ub32) u:ub8 u:ub32 (simple-array u:b32)) null)
...
(u:fn-> choose-random-candidates (state (simple-array u:b32) u:ub8 u:ub16) null)
...
(u:fn-> update-origins (state (simple-array u:b32) u:ub32 (or (simple-array u:f64) null)) (values))
...
(defun %harrison (state round candidate-count metrics indices)
  (dotimes (counter (length indices))
    (let* ((index (aref indices counter))
           ...
           (candidates (make-array (+ neighbor-count candidate-count) :element-type 'u:b32)))
      (when (plusp neighbor-count)
        (let ((neighbors (make-array neighbor-count :element-type 'u:ub32)))
...
(defun harrison (file-path &key width height indexed-p (rounds 3) (candidate-count 20)
                             (kernel-size 1) seed (parallel-p t) output-path)
  (int::check-file-exists output-path)
  (int::check-image-dimension :width width)
  (int::check-image-dimension :height height)
  (unless (typep kernel-size '(integer 1 255))
    (error 'int::invalid-kernel-size :value kernel-size))
  (unless (typep rounds '(integer 1 255))
    (error 'int::invalid-harrison-rounds :value rounds))
