      ...
      (dotimes (y (height grid))
        (dotimes (x (width grid))
          (let ((point (make-point x y z)))
            (setf (aref mask-data (get-index grid point)) (get-value mask point))))))
...
(defmethod get-index ((topology grid) (point point))
  (let ((width (width topology)))
    (+ (px point) (* (py point) width) (* (pz point) width (height topology)))))

(defmethod get-coords ((topology grid) (index integer))
  (let* ((width (width topology))
         (height (height topology))
         (i (truncate index width)))
    (make-point (mod index width)
                (mod i height)
                (truncate i height))))

(defmethod try-move ((topology grid) (point point) (direction integer))
  (macrolet ((modify-coord (coord dimension)
               (u:with-gensyms (point-coord)
                 (u:once-only (dimension)
                   `(symbol-macrolet ((,point-coord (,(u:symbolicate '#:p coord) point)))
                      (cond
    ...
    (let* ((mask (mask topology))
           ...
           (out-point (copy-point point)))
      (incf (px out-point) (aref (direction-set-dx directions) direction))
      (incf (py out-point) (aref (direction-set-dy directions) direction))
      (incf (pz out-point) (aref (direction-set-dz directions) direction))
