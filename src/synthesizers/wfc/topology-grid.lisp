...
(defmethod get-index ((topology grid) (point point:point))
  (let ((width (width topology)))
    (+ (point:x point) (* (point:y point) width) (* (point:z point) width (height topology)))))
...
(defmethod try-move ((topology grid) (point point:point) (direction integer))
  (macrolet ((modify-coord (coord dimension)
    ...
    (let* ((mask (mask topology))
      ...
      (incf (point:x out-point) (aref (direction-x directions) direction))
      (incf (point:y out-point) (aref (direction-y directions) direction))
      (incf (point:z out-point) (aref (direction-z directions) direction))
