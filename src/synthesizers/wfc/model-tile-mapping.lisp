...
(defun make-tile-set (mapping tiles)
  (let* ((tile-set (tpts:make-tile-set tiles))
         (tile-set-tiles (tpts:tiles tile-set))
         (o->p (tpts:offset->patterns tile-set))
...
(defmethod get-patterns ((mapping mapping) (tile tile:tile) (offset integer))
  (let ((pbo (tiles->patterns-by-offset mapping)))
    (%get-patterns (u:href pbo offset) tile)))

(defmethod get-patterns ((mapping mapping) (tile-set tpts:tile-set) (offset integer))
  (or (u:href (tpts:offset->patterns tile-set) offset)
