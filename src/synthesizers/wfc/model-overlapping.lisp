          ...
          (dotimes (d direction-count)
            (let ((l (u:dict #'eql)))
              (dotimes (p2 (length pattern-arrays))
                (let ((x (aref (top:direction-x directions) d))
                      (y (aref (top:direction-y directions) d))
                      (z (aref (top:direction-z directions) d)))
