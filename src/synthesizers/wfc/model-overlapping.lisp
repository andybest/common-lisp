          ...
          (dotimes (d direction-count)
            (let ((l (u:dict #'eql)))
              (dotimes (p2 (length pattern-arrays))
                (let ((x (aref (dir:x directions) d))
                      (y (aref (dir:y directions) d))
                      (z (aref (dir:z directions) d)))
...
(defun get-topology-mask (topology x y z)
  (let ((width (top:width topology))
        ...
        (periodicity (grid:periodicity topology)))
    (cond
      ((or (and (not (per:x periodicity))
                (>= x width))
           (and (not (per:y periodicity))
                (>= y height))
           (and (not (per:z periodicity))
                (>= z depth)))
       nil)
      (t
       (let ((x (mod x width))
