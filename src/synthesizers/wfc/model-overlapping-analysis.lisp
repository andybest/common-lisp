...
(defun try-extract (sample point1 point2)
  (let* ((topology (top:topology sample))
    ...
    (dotimes (tx nx)
      (let ((sx (mod (+ (point:x point2) tx) width)))
        (dotimes (ty ny)
          (let ((sy (mod (+ (point:y point2) ty) height)))
            (dotimes (tz nz)
              (let* ((sz (mod (+ (point:z point2) tz) depth))
                     (point-s (point:point sx sy sz))
...
(defun get-patterns (sample point periodicity pattern-indices pattern-arrays frequencies)
  (let* ((topology (top:topology sample))
         ...
         (height (top:height topology))
         (depth (top:depth topology))
         (max-x (if (top:periodic-x periodicity) (1- width) (- width (point:x point))))
         (max-y (if (top:periodic-y periodicity) (1- height) (- height (point:y point))))
         (max-z (if (top:periodic-z periodicity) (1- depth) (- depth (point:z point)))))
    (dotimes (x (1+ max-x))
      (dotimes (y (1+ max-y))
        (dotimes (z (1+ max-z))
          (u:when-let ((pattern-array (try-extract sample point (point:point x y z))))
