...
(defun get-patterns (sample point periodicity pattern-indices pattern-arrays frequencies)
  (let* ((topology (top:topology sample))
         ...
         (height (top:height topology))
         (depth (top:depth topology))
         (max-x (if (per:x periodicity) (1- width) (- width (point:x point))))
         (max-y (if (per:y periodicity) (1- height) (- height (point:y point))))
         (max-z (if (per:z periodicity) (1- depth) (- depth (point:z point)))))
    (dotimes (x (1+ max-x))
      (dotimes (y (1+ max-y))
        (dotimes (z (1+ max-z))
