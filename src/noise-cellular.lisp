...
(defun-gpu cellular/simplex ((p :vec3)
  ...
  (multiple-value-bind (pi pi1 pi2 v1234-x v1234-y v1234-z) (simplex/get-corner-vectors p)
    (multiple-value-bind (x y z) (funcall hash-fn pi pi1 pi2)
      (let ((jitter-window (* 0.059786577 +simplex-3d/inverse-pyramid-height+)))
        (setf x (* (cellular-weight-samples x) jitter-window)
              y (* (cellular-weight-samples y) jitter-window)
              z (* (cellular-weight-samples z) jitter-window))
        (multf v1234-x (vec4 +simplex-3d/inverse-pyramid-height+))
        (multf v1234-y (vec4 +simplex-3d/inverse-pyramid-height+))
        (multf v1234-z (vec4 +simplex-3d/inverse-pyramid-height+))
        (incf v1234-x x)
        (incf v1234-y y)
        (incf v1234-z z)
        (let ((dist-sq (+ (* v1234-x v1234-x) (* v1234-y v1234-y) (* v1234-z v1234-z))))
          (min (min (.x dist-sq) (.y dist-sq))
               (min (.z dist-sq) (.w dist-sq))))))))

(defun-gpu cellular/simplex ((p :vec3))
  (cellular/simplex p (lambda ((x :vec3) (y :vec3) (z :vec3))
