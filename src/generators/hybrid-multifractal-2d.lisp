...
;;;; 2-dimensional "hybrid" multifractal fractal noise generator

(defpackage #:%mfiano.graphics.procgen.cricket.generators.hybrid-multifractal-2d
  (:local-nicknames
...
(in-package #:%mfiano.graphics.procgen.cricket.generators.hybrid-multifractal-2d)

(defstruct (gen:hybrid-multifractal-2d
            (:include int:sampler)
            (:conc-name "")
            (:predicate nil)
            (:copier nil))
...
(defun gen:hybrid-multifractal-2d (&key seed (generator #'gen:open-simplex2s-2d) (octaves 4)
...
octaves (optional, default 0.25)."
  (unless (typep octaves '(integer 1 32))
    (error 'int:invalid-fractal-octave-count :sampler-type 'hybrid-multifractal-2d :value octaves))
...
(defmethod int:sample ((sampler gen:hybrid-multifractal-2d) x &optional (y 0d0) (z 0d0) (w 0d0))
  (declare (ignore z w))
