...
(defstruct (open-simplex2s-3d
            ...
            (:predicate nil)
            (:copier nil))
  (gradients (make-array 6144 :element-type 'u:f64) :type (simple-array u:f64 (6144)))
...
(defun open-simplex2s-3d (&key seed (orientation :standard))
  (u:mvlet* ((rng (int::make-rng seed))
             (gradients table (permute rng)))
    (%open-simplex2s-3d :rng rng
                        :gradients gradients
                        :table table
                        :orientation orientation)))

(defmethod int::sample ((sampler open-simplex2s-3d) x &optional (y 0d0) (z 0d0) (w 0d0))
  (declare (ignore w)
           (optimize speed)
           (int::f50 x y z w))
  (u:mvlet* ((gradients (gradients sampler))
    ...
    (u:while c
      (let* ((dxr (+ xri (dxr c)))
             (dyr (+ yri (dyr c)))
             (dzr (+ zri (dzr c)))
             (attn (- 0.75 (expt dxr 2) (expt dyr 2) (expt dzr 2))))
        (if (minusp attn)
            (setf c (next/fail c))
            (let* ((pxm (logand (+ xrb (xrv c)) 2047))
              ...
              (setf attn (expt attn 2)
                    c (next/success c))
              (incf value (* (expt attn 2) (+ grad-x grad-y grad-z)))))))
    (float value 1f0)))
