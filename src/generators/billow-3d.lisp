...
;;;; 3-dimensional "billow" fractal noise generator

(defpackage #:%mfiano.graphics.procgen.cricket.generators.billow-3d
  (:local-nicknames
...
(in-package #:%mfiano.graphics.procgen.cricket.generators.billow-3d)

(defstruct (gen:billow-3d
            (:include int:sampler)
            (:conc-name "")
            (:predicate nil)
            (:copier nil))
...
(defun gen:billow-3d (&key seed (generator #'gen:open-simplex2s-3d) (octaves 4) (frequency 1.0)
...
octaves (optional, default 0.5)."
  (unless (typep octaves '(integer 1 32))
    (error 'int:invalid-fractal-octave-count :sampler-type 'billow-3d :value octaves))
  (unless (realp frequency)
    (error 'int:invalid-real-argument
           :sampler-type 'billow-3d
           :argument :frequency
           :value frequency))
...
(defmethod int:sample ((sampler gen:billow-3d) x &optional (y 0d0) (z 0d0) (w 0d0))
  (declare (ignore w))
