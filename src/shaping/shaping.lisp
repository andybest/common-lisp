...
(defpackage #:mfiano.math.origin.shaping
  (:local-nicknames
...
(in-package #:mfiano.math.origin.shaping)
...
(defun circular-in-out (x)
  (declare (optimize speed))
  (if (< x 0.5)
      (* 0.5 (- 1 (sqrt (the (u:f32 0.0) (- 1 (* 4 x x))))))
      (* 0.5 (1+ (sqrt (the (u:f32 0.0) (- (* (- (* 2 x) 3) (1- (* 2 x))))))))))
