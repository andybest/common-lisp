...
(defun  merge ((d1 :float)
               (d2 :float))
  (min d1 d2))

(defun  merge/smooth ((d1 :float)
                      (d2 :float)
                      (k :float))
  (let ((h (saturate (+ 0.5 (* 0.5 (/ (- d2 d1) k))))))
    (- (mix d2 d1 h) (* k h (- 1 h)))))

(defun  merge/exclude ((d1 :float)
                       (d2 :float))
  (min (max (- d1) d2) (max (- d2) d1)))

(defun  subtract ((d1 :float)
                  (d2 :float))
  (max (- d1) d2))
...
(defun  rotate/ccw ((p :vec2)
                    (a :float))
  (let ((m (mat2 (cos a) (sin a) (- (sin a)) (cos a))))
    (* p m)))

(defun  rotate/cw ((p :vec2)
                   (a :float))
  (let ((m (mat2 (cos a) (- (sin a)) (sin a) (cos a))))
    (* p m)))

(defun  translate ((p :vec2)
                   (v :vec2))
  (- p v))
