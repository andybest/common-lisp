...
(define-function perlin ((point :vec2))
  (perlin point (lambda ((x :vec2)) (umbra.hash:fast32/2-per-corner x))))
...
(define-function perlin/derivs ((point :vec2)
  ...
  (mvlet* ((cell (floor point))
           ...
           (blend (umbra.shaping:quintic-curve/interpolate-derivative (.xy vecs)))
           (out (+ dotval0-grad0
                   (* (.x blend) k0-gk0)
...
(define-function perlin/derivs ((point :vec2))
  (perlin/derivs point (lambda ((x :vec2)) (umbra.hash:fast32/2-per-corner x))))
...
(define-function perlin-surflet ((point :vec2)
  ...
  (mvlet* ((cell (floor point))
           ...
           (out (dot (umbra.shaping:falloff-squared-c2 (min (vec4 1) vecs-squared))
...
(define-function perlin-surflet ((point :vec2))
  (perlin-surflet point (lambda ((x :vec2)) (umbra.hash:fast32/2-per-corner x))))
...
(define-function perlin-surflet/derivs ((point :vec2))
  (perlin-surflet/derivs point (lambda ((x :vec2)) (umbra.hash:fast32/2-per-corner x))))
...
(define-function perlin-improved ((point :vec2))
  (perlin-improved point (lambda ((x :vec2)) (umbra.hash:fast32 x))))
...
(define-function perlin ((point :vec3))
  (perlin point (lambda ((x :vec3)) (umbra.hash:fast32/3-per-corner x))))
...
(define-function perlin/derivs ((point :vec3)
  ...
  (mvlet* ((cell (floor point))
           ...
           (blend (umbra.shaping:quintic-curve vec))
           (blend-deriv (umbra.shaping:quintic-curve/derivative vec))
           (out (+ dot0-grad0
...
(define-function perlin/derivs ((point :vec3))
  (perlin/derivs point (lambda ((x :vec3)) (umbra.hash:fast32/3-per-corner x))))
...
(define-function perlin-surflet ((point :vec3)
  ...
  (mvlet* ((cell (floor point))
           ...
           (out (* (+ (dot (umbra.shaping:falloff-squared-c2
                            (min (vec4 1) (+ vecs-squared (.z vec))))
                           temp1)
                      (dot (umbra.shaping:falloff-squared-c2
...
(define-function perlin-surflet ((point :vec3))
  (perlin-surflet point (lambda ((x :vec3)) (umbra.hash:fast32/3-per-corner x))))
...
(define-function perlin-surflet/derivs ((point :vec3))
  (perlin-surflet/derivs point (lambda ((x :vec3)) (umbra.hash:fast32/3-per-corner x))))
...
(define-function perlin-improved ((point :vec3))
  (perlin-improved point (lambda ((x :vec3)) (umbra.hash:fast32 x))))
...
(define-function perlin ((point :vec4))
  (perlin point (lambda ((x :vec4)) (umbra.hash:fast32-2/4-per-corner x))))
