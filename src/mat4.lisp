...
(defmacro with-components (((prefix matrix) &rest rest) &body body)
  (a:once-only (matrix)
    `(symbol-macrolet ((,prefix ,matrix)
                       ...
                       (,(make-accessor-symbol prefix "30") (aref ,matrix 3))
                       (,(make-accessor-symbol prefix "01") (aref ,matrix 4))
                       (,(make-accessor-symbol prefix "11") (aref ,matrix 5))
                       (,(make-accessor-symbol prefix "21") (aref ,matrix 6))
                       (,(make-accessor-symbol prefix "31") (aref ,matrix 7))
                       (,(make-accessor-symbol prefix "02") (aref ,matrix 8))
                       (,(make-accessor-symbol prefix "12") (aref ,matrix 9))
                       (,(make-accessor-symbol prefix "22") (aref ,matrix 10))
                       (,(make-accessor-symbol prefix "32") (aref ,matrix 11))
                       (,(make-accessor-symbol prefix "03") (aref ,matrix 12))
                       (,(make-accessor-symbol prefix "13") (aref ,matrix 13))
                       (,(make-accessor-symbol prefix "23") (aref ,matrix 14))
...
(define-op mat ((m00 single-float) (m01 single-float) (m02 single-float)
                ...
                (m21 single-float) (m22 single-float) (m23 single-float)
                (m30 single-float) (m31 single-float) (m32 single-float)
                (m33 single-float))
    (:out mat)
  (let ((mat (make-array 16 :element-type 'single-float :initial-element 0f0)))
    (with-components ((%m mat))
      (psetf %m00 m00 %m01 m01 %m02 m02 %m03 m03
             %m10 m10 %m11 m11 %m12 m12 %m13 m13
             %m20 m20 %m21 m21 %m22 m22 %m23 m23
             %m30 m30 %m31 m31 %m32 m32 %m33 m33))
    mat))
...
(define-op set-projection/orthographic ((left single-float)
                                        (right single-float)
                                        (bottom single-float)
                                        (top single-float) (near single-float)
                                        (far single-float))
