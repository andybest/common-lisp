...
(defun* (translation-to-vec3! -> v3:vec) ((out v3:vec) (dquat dquat))
  (let ((s (q:zero))
        (c (q:zero)))
...
(declaim (inline translation-to-vec3))
(defun* (translation-to-vec3 -> v3:vec) ((dquat dquat))
  (translation-to-vec3! (v3:zero) dquat))

(declaim (inline translation-from-vec3!))
(defun* (translation-from-vec3! -> dquat) ((out dquat) (vec v3:vec))
...
(declaim (inline translation-from-vec3))
(defun* (translation-from-vec3 -> dquat) ((vec v3:vec))
...
(defun* (rotation-to-quat! -> q:quat) ((out q:quat) (dquat dquat))
  (with-components ((d dquat))
    (q:copy! out d.r))
  out)

(declaim (inline rotation-to-quat))
(defun* (rotation-to-quat -> q:quat) ((dquat dquat))
  (rotation-to-quat! (q:zero) dquat))

(declaim (inline rotation-from-quat!))
(defun* (rotation-from-quat! -> dquat) ((out dquat) (quat q:quat))
...
(declaim (inline rotation-from-quat))
(defun* (rotation-from-quat -> dquat) ((quat q:quat))
...
(defun* (to-mat4! -> m4:matrix) ((out m4:matrix) (dquat dquat))
...
(defun* (to-mat4 -> m4:matrix) ((dquat dquat))
  (to-mat4! (m4:id) dquat))

(defun* (from-mat4! -> dquat) ((out dquat) (matrix m4:matrix))
...
(defun* (from-mat4 -> dquat) ((matrix m4:matrix))
  (from-mat4! (id) matrix))

(defun* (to-screw -> (values single-float single-float v3:vec v3:vec))
    ((dquat dquat))
...
(defun* (from-screw! -> dquat) ((out dquat) (angle single-float)
...
(defun* (from-screw -> dquat) ((angle single-float) (pitch single-float)
...
(defun* (sclerp! -> dquat) ((out dquat) (dquat1 dquat) (dquat2 dquat)
  ...
  (let ((diff (* (inverse dquat1) dquat2)))
    (multiple-value-bind (angle pitch direction moment) (to-screw diff)
      (*! out dquat1 (from-screw (cl:* angle factor)
                                 (cl:* pitch factor)
                                 direction
                                 moment))))
  out)
