...
units, to radian units."
  (declare (optimize speed))
  (com:cwset 3 out vec (cl:* vec const:+deg/double+))
  out)
...
units, to degree units."
  (declare (optimize speed))
  (com:cwset 3 out vec (cl:* vec const:+rad/double+))
  out)
...
(defun sqrt! (out vec)
  ...
  (declare (optimize speed))
  (com:cwset 3 out vec (cl:sqrt (the (u:f64 0d0) vec)))
  out)
...
VEC."
  (declare (optimize speed))
  (com:cwset 3 out vec (cl:asin (the (u:f64 -1d0 1d0) vec)))
  out)
...
vector VEC."
  (declare (optimize speed))
  (com:cwset 3 out vec (cl:acos (the (u:f64 -1d0 1d0) vec)))
  out)
