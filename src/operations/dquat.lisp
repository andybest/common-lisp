...
(defun from-mat3! (out mat)
  ...
  (with-components ((o out))
    (dm3:with-components ((m mat))
      (psetf ow (cl:* (sqrt (the (double-float 0d0)
                                 (max 0d0 (cl:+ m00 m11 m22 1d0))))
                      0.5d0)
             ox (cl:* (sqrt (the (double-float 0d0)
                                 (max 0d0 (cl:- (cl:+ m00 1d0) m11 m22))))
                      (signum (cl:- m21 m12))
                      0.5d0)
             oy (cl:* (sqrt (the (double-float 0d0)
                                 (max 0d0 (cl:- (cl:+ (cl:- m00 1d0) m11) m22))))
                      (signum (cl:- m02 m20))
                      0.5d0)
             oz (cl:* (sqrt (the (double-float 0d0)
                                 (max 0d0 (cl:+ (cl:- 1d0 m00 m11) m22))))
                      (signum (cl:- m10 m01))
                      0.5d0))))
  out)
