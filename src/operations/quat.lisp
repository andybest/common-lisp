      ...
      (ecase space
        ...
        (:world
         (psetf ow (cl:- (cl:* rw qw) (cl:* rx qx) (cl:* ry qy) (cl:* rz qz))
                ...
                oz (cl:- (cl:* rw qz) (cl:* rz qw) (cl:* rx qy)
                         (cl:* ry qx)))))))
  out)
...
(defun from-mat3! (out mat)
  ...
  (with-components ((o out))
    (m3:with-components ((m mat))
      (psetf ow (cl:* (sqrt (the (single-float 0.0)
                                 (max 0.0 (cl:+ m00 m11 m22 1.0))))
                      0.5)
             ox (cl:* (sqrt (the (single-float 0.0)
                                 (max 0.0 (cl:- (cl:+ m00 1.0) m11 m22))))
                      (signum (cl:- m21 m12))
                      0.5)
             oy (cl:* (sqrt (the (single-float 0.0)
                                 (max 0.0 (cl:- (cl:+ (cl:- m00 1.0) m11) m22))))
                      (signum (cl:- m02 m20))
                      0.5)
             oz (cl:* (sqrt (the (single-float 0.0)
                                 (max 0.0 (cl:+ (cl:- 1.0 m00 m11) m22))))
                      (signum (cl:- m10 m01))
                      0.5))))
  out)
