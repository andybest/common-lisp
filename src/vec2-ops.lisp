...
(ss:defspecialization (vec :inline t) ((xy net.mfiano.lisp.origin.dvec2:vec))
    vec
  (net.mfiano.lisp.origin.dvec2:with-components ((v xy))
    (%vec (float vx 1f0) (float vy 1f0))))

;;; constants

(u:define-constant +zero+ (%vec 0f0 0f0) :test #'equalp)

(u:define-constant +up+ (%vec 0f0 1f0) :test #'equalp)

(u:define-constant +down+ (%vec 0f0 -1f0) :test #'equalp)

(u:define-constant +left+ (%vec -1f0 0f0) :test #'equalp)

(u:define-constant +right+ (%vec 1f0 0f0) :test #'equalp)
...
(int:define-op from-radians! ((out vec)
                              (radians (single-float
                                        #.(cl:- const:pi)
                                        #.const:pi)))
    (:out vec)
  (with-components ((o out))
    (setf ox (cl:cos radians)
          oy (cl:sin radians))
    out))

(int:define-op from-radians ((radians single-float
                                      #.(cl:- const:pi)
                                      #.const:pi))
    (:out vec)
...
(int:define-op from-degrees! ((out vec) (degrees (single-float -360f0 360f0)))
    (:out vec)
  (with-components ((o out))
    (setf ox (cl:cos (cl:* degrees const:+deg+))
          oy (cl:sin (cl:* degrees const:+deg+)))
    out))

(int:define-op from-degrees ((degrees (single-float -360f0 360f0))) (:out vec)
...
(int:define-op mod ((in vec) (divisor single-float)) (:out vec)
  (mod! (vec) in divisor))
