...
(ss:defspecialization (quat! :inline t) ((out quat)) quat
  (with-components ((o out))
    (psetf ow 0f0 ox 0f0 oy 0f0 oz 0f0))
  out)
...
(ss:defspecialization (quat! :inline t) ((out quat) (w real)) quat
  (with-components ((o out))
    (psetf ow (float w 1f0) ox 0f0 oy 0f0 oz 0f0))
  out)
...
(ss:defspecialization (quat! :inline t) ((out quat)
    ...
    quat
  (with-components ((o out))
    (psetf ow (float w 1f0) ox (float x 1f0) oy (float y 1f0) oz (float z 1f0)))
  out)
...
(ss:defspecialization (quat! :inline t) ((out quat) (xyz v3:vec)) quat
  (with-components ((o out))
    (v3:with-components ((v xyz))
      (psetf ow 0f0 ox vx oy vy oz vz)))
  out)
...
(ss:defspecialization (quat! :inline t) ((out quat) (xyzw v4:vec)) quat
  (with-components ((o out))
    (v4:with-components ((v xyzw))
...
(ss:defspecialization (quat! :inline t)
    ((out quat) (quat net.mfiano.lisp.origin.dquat:quat))
    quat
  (with-components ((o out))
    (net.mfiano.lisp.origin.dquat:with-components ((q quat))
      (psetf ow (float qw 1f0)
             ox (float qx 1f0)
             oy (float qy 1f0)
             oz (float qz 1f0))))
  out)
...
(int:define-op = ((in1 quat) (in2 quat)) (:out boolean)
...
(int:define-op ~ ((in1 quat) (in2 quat) &key (tolerance single-float 1e-7))
    (:out boolean)
  (with-components ((q1 in1) (q2 in2))
...
(int:define-op to-euler! ((out v3:vec) (in quat)) (:out v3:vec :speed nil)
  (with-components ((q in))
...
(int:define-op to-euler ((in quat)) (:out v3:vec :speed nil)
  (to-euler! (v3:vec) in))

(int:define-op to-vec3! ((out v3:vec) (in quat)) (:out v3:vec)
  (v3:with-components ((v out))
    (with-components ((q in))
      (psetf vx qx vy qy vz qz)))
  out)

(int:define-op to-vec3 ((in quat)) (:out v3:vec)
  (to-vec3! (v3:vec) in))

(int:define-op to-vec4! ((out v4:vec) (in quat)) (:out v4:vec)
  (v4:with-components ((v out))
    (with-components ((q in))
      (psetf vx qw vy qx vz qy vw qz)))
  out)

(int:define-op to-vec4 ((in quat)) (:out v4:vec)
  (to-vec4! (v4:vec) in))

(int:define-op from-vec3! ((out quat) (in v3:vec)) (:out quat)
  (with-components ((q out))
    (v3:with-components ((v in))
      (psetf qw 0f0 qx vx qy vy qz vz)))
  out)

(int:define-op from-vec3 ((in v3:vec)) (:out quat)
  (from-vec3! (quat) in))

(int:define-op from-vec4! ((out quat) (in v4:vec)) (:out quat)
  (with-components ((q out))
    (v4:with-components ((v in))
      (psetf qw vx qx vy qy vz qz vw)))
  out)

(int:define-op from-vec4 ((in v4:vec)) (:out quat)
  (from-vec4! (quat) in))
...
(int:define-op from-mat3! ((out quat) (in m3:mat))
...
(int:define-op from-mat4! ((out quat) (in m4:mat)) (:out quat :speed nil)
  (with-components ((o out))
...
(int:define-op from-mat4 ((in m4:mat)) (:out quat :speed nil)
  (from-mat4! (quat 1) in))
