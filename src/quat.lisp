...
(declaim (inline ->v3!))
(defun* (->v3! -> v3:vec) ((out v3:vec) (quat quat))
...
(declaim (inline ->v3))
(defun* (->v3 -> v3:vec) ((quat quat))
  (->v3! (v3:zero) quat))

(declaim (inline ->v4!))
(defun* (->v4! -> v4:vec) ((out v4:vec) (quat quat))
...
(declaim (inline ->v4))
(defun* (->v4 -> v4:vec) ((quat quat))
  (->v4! (v4:zero) quat))

(declaim (inline v3->q!))
(defun* (v3->q! -> quat) ((out quat) (vec v3:vec))
...
(declaim (inline v3->q))
(defun* (v3->q -> quat) ((vec v3:vec))
  (v3->q! (zero) vec))

(declaim (inline v4->q!))
(defun* (v4->q! -> quat) ((out quat) (vec v4:vec))
  (with-components ((q out))
...
(declaim (inline v4->q))
(defun* (v4->q -> quat) ((vec v4:vec))
  (v4->q! (zero) vec))

(declaim (inline ->m4!))
(defun* (->m4! -> m4:matrix) ((out m4:matrix) (quat quat))
...
(declaim (inline ->m4))
(defun* (->m4 -> m4:matrix) ((quat quat))
  (->m4! (m4:id) quat))

(defun* (m4->q! -> quat) ((out quat) (matrix m4:matrix))
  (with-components ((q out))
...
(declaim (inline m4->q))
(defun* (m4->q -> quat) ((matrix m4:matrix))
  (m4->q! (id) matrix))
