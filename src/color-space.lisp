...
(define-gpu-function rgb->grayscale ((color :vec3))
  (vec3 (+ (* (.r color) 0.2126)
           (* (.g color) 0.7152)
           (* (.b color) 0.0722))))

(define-gpu-function rgb->grayscale ((color :vec4))
  (vec4 (rgb->grayscale (.rgb color)) (.a color)))

;;; Hue

(define-gpu-function hue->rgb ((hue :float))
  (let ((v (* hue 6)))
    (saturate
     (vec3 (1- (abs (- v 3)))
           (- 2 (abs (- v 2)))
...
(define-gpu-function rgb->hcv ((color :vec3))
...
(define-gpu-function rgb->hcv ((color :vec4))
  (vec4 (rgb->hcv (.rgb color)) (.a color)))

;;; HSV (hue/saturation/value)

(define-gpu-function rgb->hsv ((color :vec3))
  (let* ((hcv (rgb->hcv color))
         (s (/ (.y hcv) (+ (.z hcv) +epsilon+))))
    (vec3 (.x hcv) s (.z hcv))))

(define-gpu-function rgb->hsv ((color :vec4))
  (vec4 (rgb->hsv (.rgb color)) (.a color)))

(define-gpu-function hsv->rgb ((color :vec3))
  (let ((rgb (hue->rgb (.x color))))
    (* (1+ (* (1- rgb) (.y color))) (.z color))))

(define-gpu-function hsv->rgb ((color :vec4))
  (vec4 (hsv->rgb (.xyz color)) (.a color)))

;;; HCY (hue/saturation/luminance)

(define-gpu-function rgb->hcy ((color :vec3))
...
(define-gpu-function rgb->hcy ((color :vec4))
  (vec4 (rgb->hcy (.rgb color)) (.a color)))

(define-gpu-function hcy->rgb ((color :vec3))
  (let* ((hcy-weights (vec3 0.299 0.587 0.114))
         (rgb (hue->rgb (.x color)))
...
(define-gpu-function hcy->rgb ((color :vec4))
  (vec4 (hcy->rgb (.xyz color)) (.a color)))

;;; HSL (hue/saturation/lightness)

(define-gpu-function rgb->hsl ((color :vec3))
  (let* ((hcv (rgb->hcv color))
         (l (- (.z hcv) (* (.y hcv) 0.5)))
         (s (/ (.y hcv) (- 1 (+ (abs (1- (* l 2)))) +epsilon+))))
    (vec3 (.x hcv) s l)))

(define-gpu-function rgb->hsl ((color :vec4))
  (vec4 (rgb->hsl (.rgb color)) (.a color)))

(define-gpu-function hsl->rgb ((color :vec3))
  (let ((rgb (hue->rgb (.x color)))
        (c (* (- 1 (abs (1- (* 2 (.z color))))) (.y color))))
    (+ (* (- rgb 0.5) c) (.z color))))

(define-gpu-function hsl->rgb ((color :vec4))
  (vec4 (hsl->rgb (.xyz color)) (.a color)))

;;; SRGB

(define-gpu-function rgb->srgb-approx ((color :vec3))
  (expt (.rgb color) (vec3 +gamma+)))

(define-gpu-function rgb->srgb-approx ((color :vec4))
  (vec4 (rgb->srgb-approx (.rgb color)) (.a color)))

(define-gpu-function rgb->srgb ((color :vec3))
  (mix (* 12.92 color)
       (- (* 1.055 (expt color (vec3 (/ 2.4)))) 0.055)
       (step (vec3 0.0031308) color)))

(define-gpu-function rgb->srgb ((color :vec4))
  (vec4 (rgb->srgb (.rgb color)) (.a color)))

(define-gpu-function srgb->rgb-approx ((color :vec3))
  (expt color (vec3 +gamma-inverse+)))

(define-gpu-function srgb->rgb-approx ((color :vec4))
  (vec4 (srgb->rgb-approx (.rgb color)) (.a color)))

(define-gpu-function srgb->rgb ((color :vec3))
  (mix (/ color 12.92)
       (expt (/ (+ color 0.055) 1.055) (vec3 2.4))
       (step (vec3 0.04045) color)))

(define-gpu-function srgb->rgb ((color :vec4))
  (vec4 (srgb->rgb (.rgb color)) (.a color)))

;;; CIE XYZ 1931

(define-gpu-function rgb->xyz ((color :vec3))
  (let ((transform (mat3 0.4124564 0.3575761 0.1804375
                         0.2126729 0.7151522 0.072175
                         0.0193339 0.119192 0.9503041)))
    (* transform color)))

(define-gpu-function rgb->xyz ((color :vec4))
  (vec4 (rgb->xyz (.rgb color)) (.a color)))

(define-gpu-function xyz->rgb ((color :vec3))
  (let ((transform (mat3 3.2404542 -1.5371385 -0.4985314
                         -0.969266 1.8760108 0.041556
                         0.0556434 -0.2040259 1.0572252)))
    (* transform color)))

(define-gpu-function xyz->rgb ((color :vec4))
  (vec4 (xyz->rgb (.xyz color)) (.a color)))

;;; CIE xyY

(define-gpu-function xyy->xyz ((color :vec3))
  (let* ((y (.z color))
         (x (/ (* y (.x color)) (.y color)))
         (z (/ (* y (- 1 (.x color) (.y color))) (.y color))))
    (vec3 x y z)))

(define-gpu-function xyy->xyz ((color :vec4))
  (vec4 (xyy->xyz (.xyz color)) (.a color)))

(define-gpu-function xyz->xyy ((color :vec3))
  (let* ((v (+ (.x color) (.y color) (.z color)))
         (x (/ (.x color) v))
         (y (/ (.y color) v)))
    (vec3 x y (.y color))))

(define-gpu-function xyz->xyy ((color :vec4))
  (vec4 (xyz->xyy (.xyz color)) (.a color)))

(define-gpu-function rgb->xyy ((color :vec3))
  (xyz->xyy (rgb->xyz color)))

(define-gpu-function rgb->xyy ((color :vec4))
  (xyz->xyy (rgb->xyz color)))

(define-gpu-function xyy->rgb ((color :vec3))
  (xyz->rgb (xyy->xyz color)))

(define-gpu-function xyy->rgb ((color :vec4))
  (xyz->rgb (xyy->xyz color)))
