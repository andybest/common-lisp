...
(defun make-points (point-pairs)
  (unless (>= (length point-pairs) 4)
    (error "A curve modifier requires at least 4 control points."))
  (unless (and (listp point-pairs)
               (every (lambda (x) (typep x '(cons real real))) point-pairs))
    (error "Control point must be a cons of 2 real numbers."))
...
(defun mod:curve (source &rest points)
...
(defmethod int:sample ((sampler curve) x &optional (y 0d0) (z 0d0) (w 0d0))
  (let* ((control-points (control-points sampler))
         ...
         (index (u:clamp (position sample control-points :test #'< :key #'car) 2 count))
         (index1 (u:clamp (- index 2) 0 (1- count)))
         (index2 (u:clamp (1- index) 0 (1- count)))
         (index3 (u:clamp index 0 (1- count)))
         (index4 (u:clamp (1- index) 0 (1- count))))
    (if (= index2 index3)
