...
(diag "copy")
(with-quats ((q (quat 0.34003425 -0.4920528 0.8754709 0.6535034))
             (o (quat)))
  (is (qcp! o q) q)
  (is o q)
  (is (qcp q) q)
...
(with-quat (q (quat 0.3628688 0.9540863 0.017128706 0.32979298))
  (with-vec3s ((r (vec3 qx qy qz))
               (o (v3zero)))
    (is (q->v3! o q) r)
    (is o r)
    (is (q->v3 q) r)))
(with-vec3 (v (vec3 0.2571392 0.19932675 -0.025900126))
...
(with-quat (q (quat 0.3628688 0.9540863 0.017128706 0.32979298))
  (with-vec3s ((r (vec4 qw qx qy qz))
               (o (v4zero)))
    (is (q->v4! o q) r)
    (is o r)
    (is (q->v4 q) r)))
(with-vec4 (v (vec4 0.2571392 0.19932675 -0.025900126 0.8267517))
  (with-quats ((r (quat vx vy vz vw))
               (o (quat)))
    (is (v4->q! o v) r)
    (is o r)
    (is (v4->q v) r)))
...
(with-quats ((q (qrot +qid+ (vec3 (/ pi 3) 0 0)))
             (qo (quat)))
  (with-matrices ((r (matrix 1 0 0 0 0 0.5 -0.86602545 0 0 0.86602545 0.5 0 0 0 0
                             1))
                  (mo (mid)))
    (ok (m~ (q->m! mo q) r))
    (ok (m~ mo r))
