...
(define-test m3/clamp
  (let ((m (m3:mat 1 -2 3 -4 5 -6 7 -8 9))
        (o (m3:mat))
        (r (m3:mat 1 -1 1 -1 1 -1 1 -1 1)))
    (is m3:= (m3:clamp! o m -1.0 1.0) r)
    (is m3:= o r)
    (is m3:= (m3:clamp m -1.0 1.0) r)
    (is m3:= (m3:clamp m
...
(define-test m3/add
  (let ((m1 (m3:mat 1 9 4 2 7 9 1 5 2))
        (m2 (m3:mat 9 2 3 8 3 8 1 0 1))
        (o (m3:mat))
        (r (m3:mat 10 11 7 10 10 17 2 5 3)))
    (is m3:= (m3:+! o m1 m2) r)
    (is m3:= o r)
    (is m3:= (m3:+ m1 m2) r)
...
(define-test m3/multiply
  (let ((m1 (m3:mat 1 5 9 2 6 10 3 7 11))
        (m2 (m3:mat 10 50 90 20 60 100 30 70 110))
        (m3 m3:+id+)
        (r (m3:mat 38 98 158 44 116 188 50 134 218))
        (rot-z (m3:rotate m3:+id+ const:pi/3))
        (tr1 (m3:translate m3:+id+ (v2:vec 5 10)))
        (tr2 (m3:translate m3:+id+ (v2:vec 10 20)))
        (o (m3:mat)))
    (is m3:= (m3:*! o m1 m1) r)
    (is m3:= o r)
    (is m3:= (m3:* m1 m3) m1)
    ...
    (is v2:= (m3:get-translation (m3:* tr1 tr2)) (v2:vec 15 30))
...
(define-test m3/rotation-copy
  (let ((m (m3:mat 1 5 0 2 6 0 0 0 1))
        (r (m3:mat 1 5 0 2 6 0 0 0 1))
        (o (m3:mat 1)))
...
(define-test m3/scale
  (let ((m (m3:mat 10 0 0 0 20 0 0 0 2))
        (o (m3:mat 1))
        (s (m3:mat 10 0 0 0 40 0 0 0 2))
        (v (v2:vec 1 2)))
...
(define-test m3/vec3-multiply
  (let ((m (m3:rotate m3:+id+ const:pi/3))
        (v (v3:vec 1 2 3))
        (o (v3:vec))
        (rv (v3:vec -1.2320509 1.8660254 3)))
...
(define-test m3/trace
  ...
  (is = (m3:trace m3:+id+) 3)
  (is = (m3:trace (m3:mat 1 2 3 4 5 6 7 8 9)) 15))
