...
(defvar *dump-spec* '(:streaminfo :application :seektable :vorbis-comment :cuesheet :picture))
...
(defmethod dump ((type (eql :streaminfo)) node level)
  (with-slots (minimum-block-size maximum-block-size minimum-frame-size
               ...
               sample-count md5)
      (data node)
    (print-line level "Stream Information" (format nil "~:d bytes" (%metadata-size (header node))))
    (print-line (+ level 2) "Minimum block size" (format nil "~:d bytes" minimum-block-size))
    (print-line (+ level 2) "Maximum block size" (format nil "~:d bytes" maximum-block-size))
    (print-line (+ level 2) "Minimum frame size" (format nil "~:d bytes" minimum-frame-size))
    (print-line (+ level 2) "Maximum frame size" (format nil "~:d bytes" maximum-frame-size))
    (print-line (+ level 2) "Sample rate" (format nil "~:dHz" sample-rate))
    (print-line (+ level 2) "Channels" channel-count)
    (print-line (+ level 2) "Bit depth" (format nil "~a bits per sample" (1+ bits-per-sample)))
    (print-line (+ level 2) "Total samples" (format nil "~:d" sample-count))
    (print-line (+ level 2) "MD5 checksum" (format nil "~{~2,'0x~}" (coerce md5 'list)))))

(defmethod dump ((type (eql :padding)) node level)
  (print-line level "Padding" (format nil "~:d bytes" (%metadata-size (header node)))))

(defmethod dump ((type (eql :application)) node level)
  (print-line level "Application" (format nil "~:d bytes" (%metadata-size (header node))))
  (with-slots (id) (data node)
    (print-line (+ level 2) "ID" id)))

(defmethod dump ((type (eql :seektable)) node level)
  (print-line level "Seek Table" (format nil "~:d bytes" (%metadata-size (header node))))
  (with-slots (seek-points) (data node)
    (dolist (seek-point seek-points)
      (with-slots (%sample %offset %sample-count) seek-point
        ...
        (print-line (+ level 4) "Target sample" (format nil "~:d" %sample))
        (print-line (+ level 4) "Offset" (format nil "~:d" %offset))
        (print-line (+ level 4) "Sample count" (format nil "~:d" %sample-count))))))

(defmethod dump ((type (eql :vorbis-comment)) node level)
  (print-line level "Vorbis Comment" (format nil "~:d bytes" (%metadata-size (header node))))
  (with-slots (vendor user-comments-count user-comments) (data node)
    (print-line (+ level 2) "Vendor" vendor)
    (print-line (+ level 2) "Comments count" (format nil "~:d" user-comments-count))
    (dolist (comment (sort (u:hash->alist user-comments) #'string< :key #'car))
      (destructuring-bind (key . value) comment
        (print-line (+ level 4) key (if (zerop (length value)) "<empty>" value))))))

(defmethod dump ((type (eql :cuesheet)) node level)
  (print-line level "Cue Sheet" (format nil "~:d bytes" (%metadata-size (header node))))
  (with-slots (media-catalog-id lead-in-sample-count cd-flag track-count tracks) (data node)
    (print-line (+ level 2) "Media catalog ID" (remove #\Nul media-catalog-id))
    (print-line (+ level 2) "Lead-in samples" (format nil "~:d" lead-in-sample-count))
    (print-line (+ level 2) "Compact disc?" (if (zerop cd-flag) "No" "Yes"))
    (print-line (+ level 2) "Tracks" track-count)
    (dolist (track tracks)
      (with-slots (%offset %number %isrc %type %pre-emphasis-flag %index-count %indices) track
        ...
        (print-line (+ level 4) "Pre-emphasis" (if (zerop %pre-emphasis-flag) "No" "Yes"))
        (print-line (+ level 4) "Indices" %index-count)
        (dolist (index %indices)
          (with-slots (%offset %number) index
            (print-line (+ level 6) "Offset" (format nil "~:d" %offset))
...
(defmethod dump ((type (eql :picture)) node level)
  ...
  (with-slots (type mime-type description width height bits-per-pixel indexed-color-count size)
      (data node)
